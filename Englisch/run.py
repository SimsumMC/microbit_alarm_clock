...
# Sayings (translated from German to English by deepl.com)

# https://the-heros-journey.at/365-lebensweisheiten/

Sayings = ["Small things are responsible for big changes",
           ...
           "If you want to change your country, you must first change your village, if you want to change your village, you must first change your family, and if you want to change your family, you must first change yourself",                  "To get started, you have to stop talking and start acting.",
...
def alarm_check(): # checks if a alarm is set
...
def delete_alarm(): # deletes the current alarm
    global hours, active, minutes
    if active == True:
        display.scroll("B to cancel")
        sleep(1000)
        ...
        display.scroll("There is no alarm set!")


def showsentence(sentence="undefined"): # function to show sayings word per word to cancel earlier 
...
def set_hours(): # to set the hours for an alarm
    global hours
...
def set_minutes(): # to set the minutes for an alarm
    global minutes
...
while True:
    ...
    if alarm_check():
        ...
        display.clear()
        showsentence(random.choice(Sayings))
    elif button_b.get_presses():
        display.clear()
        set_clock()
    elif pin_logo.is_touched():
